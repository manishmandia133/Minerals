...
const { normDate } = require('../../utils/dates');
...
function listRecords(req, res) {
  ...
  const since = normDate(req.query.since || req.query.from_date);
  const sortRaw = String(req.query.sort || req.query.order || '').toLowerCase();
  const newest = sortRaw === 'new' || sortRaw === 'newest' || req.query.latest === 'true' || req.query.latest === '1';
  let all = load();
...
function landing(req, res) {
  res.type('html').send(
    ...
    `GET  /records?limit=100&amp;since=2024-01-01&amp;sort=newest\nGET  /records/:id\nGET  /health</pre>`);
}
